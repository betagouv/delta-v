...
import { getAllMatchingCurrencies } from '../../../../src/api/common/services/currencies/getCurrencies.service';
...
describe('test getCurrencies', () => {
  it('should return list of currencies', async () => {
    ...
    const currencies = await getAllMatchingCurrencies({
      shoppingProducts: currencyIds.map((id) => ShoppingProductFactory({ currency: id })),
      currencyRepository,
    });
  ...
  it('should throw error, missing currency', async () => {
    ...
    try {
      await getAllMatchingCurrencies({
        shoppingProducts: currencyIds.map((id) => ShoppingProductFactory({ currency: id })),
        currencyRepository,
      });
    } catch (error) {
