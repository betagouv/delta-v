...
const InputGroupComponent = ({ id, label, helperText, error, children, required, fullWidth, tooltipMessage, preventFocusOnLabelClick }: InputGroupProps) => {
  return (
    <div className={clsxm({ 'w-full': fullWidth })}>
      {label && (
        <InputGroupLabel id={id} label={label} required={required} tooltipMessage={tooltipMessage} preventFocusOnLabelClick={preventFocusOnLabelClick} />
      )}
